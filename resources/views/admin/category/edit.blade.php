...
<div class="card">
    <div class="card-body">
        <h5 class="card-title">Yangi ustun turi yangilash</h5>
        <!-- General Form Elements -->
        <form id="form-category" method="post" action="{{ route('admin.category.update', ['id' => $category->id]) }}">
            @csrf
            <div class="row mb-3">
                <label for="inputText" class="col-sm-2 col-form-label required">Tur nomi</label>
                <div class="col-sm-10">
                    <input name="name" type="text" class="form-control" value="{{ old("name", $category->value) }}">
                </div>
            </div>
        </form>
