...
<div class="card">
    <div class="card-body">
        ...
        <form id="form-group" method="post" action="{{ route('admin.attribute-group.update',['id'=>$group->id]) }}">
            @csrf
            <div class="row mb-3">
                <label for="inputText" class="col-sm-2 col-form-label required">Guruh nomi</label>
                <div class="col-sm-10">
                    <input name="name" type="text" class="form-control" value="{{old("name", $group->title)}}">
                </div>
            </div>
            <div class="row mb-3">
